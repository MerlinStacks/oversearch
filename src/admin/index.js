...
apiFetch.use(
	apiFetch.createNonceMiddleware( window.overseekSearchAdmin?.nonce )
);
				...
				<Card className="overseek-card bento-half">
					...
					<CardBody>
						<ToggleControl
							...
							onChange={ ( value ) =>
								updateSetting( 'fuzzy_enabled', value )
							}
						/>
						{ settings.fuzzy_enabled && (
							<RangeControl
								...
								onChange={ ( value ) =>
									updateSetting( 'fuzzy_threshold', value )
								}
								min={ 1 }
								max={ 3 }
							/>
						...
						<RangeControl
							...
							onChange={ ( value ) =>
								updateSetting( 'results_per_page', value )
							}
							min={ 4 }
							max={ 20 }
						/>
						<RangeControl
							...
							onChange={ ( value ) =>
								updateSetting( 'max_dropdown_results', value )
							}
							min={ 1 }
							max={ 15 }
						/>
				...
				<Card className="overseek-card bento-half">
					...
					<CardBody>
						<ToggleControl
							...
							onChange={ ( value ) =>
								updateSetting( 'highlight_matches', value )
							}
						/>
						<ToggleControl
							...
							onChange={ ( value ) =>
								updateSetting( 'voice_search', value )
							}
						/>
						<ToggleControl
							...
							onChange={ ( value ) =>
								updateSetting( 'track_analytics', value )
							}
						/>
						<ToggleControl
							...
							onChange={ ( value ) =>
								updateSetting( 'replace_search', value )
							}
						/>
					</CardBody>
				</Card>
					...
					<CardBody>
						...
						>
							<RangeControl
								...
								onChange={ ( value ) =>
									updateSetting( 'title_weight', value )
								}
								min={ 1 }
								max={ 5 }
							/>
							<RangeControl
								...
								onChange={ ( value ) =>
									updateSetting( 'sku_weight', value )
								}
								min={ 1 }
								max={ 5 }
							/>
							<RangeControl
								...
								onChange={ ( value ) =>
									updateSetting( 'description_weight', value )
								}
								min={ 1 }
								max={ 5 }
							/>
		...
		try {
			const [ summaryRes, queriesRes, noResultsRes ] = await Promise.all(
				[
					...
					apiFetch( {
						path: apiPath(
							`/analytics/queries?days=${ days }&limit=10`
						),
					} ),
					apiFetch( {
						path: apiPath(
							`/analytics/no-results?days=${ days }&limit=10`
						),
					} ),
				]
			);
	...
	return (
		<div className="overseek-analytics">
			<div className="overseek-date-filter">
				<label htmlFor="overseek-date-filter">📅 TIME PERIOD:</label>
				<select
					...
					<CardBody>
						{ topQueries.length === 0 ? (
							<div className="overseek-empty-state">
								...
								<p>
									No search data yet. Searches will appear
									here.
								</p>
							</div>
						) : (
												...
												<Button
													...
													href={ `${
														window
															.overseekSearchAdmin
															.adminUrl
													}?page=overseek-search-synonyms&add=${ encodeURIComponent(
														q.query
													) }` }
												>
													+ Add Synonym
 ...
 */
function AdminApp() {
	const { currentTab } = window.overseekSearchAdmin;
